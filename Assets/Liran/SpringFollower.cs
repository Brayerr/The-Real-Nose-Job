...
public class SpringFollower : MonoBehaviour
{
    ...
    [SerializeField] public Transform anchor;       // The pivot or "hinge" point
    ...
    public bool isAnchorFollowing = true;
    ...
    private void Update()
    {
        if (!isAnchorFollowing)
        {
            float dt = Time.deltaTime;

            // 1. Apply gravity to velocity
            velocity += Physics.gravity * gravityMod * dt;
    ...
    private void LateUpdate()
    {
        if (isAnchorFollowing)
        {
            anchor.position = transform.position + followOffset;
        }
    }
